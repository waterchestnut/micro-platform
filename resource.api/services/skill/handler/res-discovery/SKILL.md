...
| resType | string | 否 | 资源类型筛选条件，取值：book(图书)、thesis(论文)、journal(期刊)、dataset(数据)、video(视频)、patent(专利)、audio(音频)、image(图片)、standard(标准)、article(招考信息)、exercise(试题)、conference(会议)、upload(自定义上传)、camp(夏令营)、enrollment(招生)、admission(录取)、recruitment(招聘)、csc(公费留学) |
