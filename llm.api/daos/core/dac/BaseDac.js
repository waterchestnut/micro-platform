...
export default class {
     ...
     * @returns {Promise<Object>} 记录对象
     */
    async getOneByFilter(filter = {}, projection = null, options = null) {
    ...
    async getByPage(pageIndex, pageSize, options) {
        ...
        if (pageSize > 0) {
            let docs = await this._Model.find(params.$and.length ? params : {}, options.projection).sort(options.sort).skip((pageIndex - 1) * pageSize).limit(pageSize).lean().exec()
            if (!docs || !docs.length) {
                return ret
            }
            ret.rows = docs
    ...
    async getTop(length, options, sort = {updateTime: -1}) {
        ...
        let docs = await this._Model.find(params.$and.length ? params : {}, options.projection).sort(sort).limit(length).lean().exec()
        if (!docs || !docs.length) {
            return []
        }
