 ...
 */
export async function initSkillChatContext(channel, query, options = {}) {
    const result = await skillManager.generateSystemPromptForChannel(channel, query, {
        ...options,
 ...
 */
export async function checkSkillSelection(content, skillContext) {
    ...
    try {
        // 加载技能详情
        const config = skillManager.getChannelSkillsConfig(channel)
        const detailResult = await skillManager.generateSkillDetailPrompt(candidate.name, {
 ...
 */
export async function executeSkillToolCalls(toolCalls, skillContext) {
    ...
    for (const toolCall of toolCalls) {
        ...
        try {
            ...
            if (isSkillTool) {
                // 执行 Skill 指令
                const result = await executeSingleSkillTool(toolName, functionCall.arguments, matchedSkills)
                results.push({
 ...
 */
async function executeSingleSkillTool(toolName, argsJson, matchedSkills) {
    ...
    const result = await skillManager.executeSkillCommand(skillName, commandName, parameters)
 ...
 * @returns {Object}
 */
export function getSkillStats(skillContext) {
    return {
