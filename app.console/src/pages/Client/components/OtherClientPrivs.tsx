...
import {getClientPrivList, saveOtherClientPrivs} from '@/services/app/clientPriv'
import {Checkbox, CheckboxChangeEvent, Space} from 'antd'

export type OtherClientPrivsProps = {
  onSaveFinish?: (resData?: any) => Promise<void>;
  clientCode: string;
  apiRelativeUrls?: any;
};

export type OtherClientPrivsAction = {
  show: (record?: any) => void;
  close: () => void;
}

const OtherClientPrivs: ForwardRefRenderFunction<OtherClientPrivsAction, OtherClientPrivsProps> = (props, ref) => {
  const {onSaveFinish, clientCode, apiRelativeUrls} = props
  const [isOpen, setIsOpen] = useState(false)
  const [otherClientInfo, setOtherClientInfo] = useState<any>(null)
  ...
  const handleOk = async () => {
    ...
    let ret
    let tip
    ret = await saveOtherClientPrivs(clientCode, otherClientInfo.clientCode, privs, apiRelativeUrls?.saveOtherClientPrivs)
    tip = '修改权限'
    if (ret.code !== 0) {
  ...
  return (
    <ModalForm
      title={`${otherClientInfo?.clientName}赋权`}
      open={isOpen}
      layout={'vertical'}
      formRef={formRef}
      modalProps={
...
export default React.forwardRef(OtherClientPrivs)
