    ...
    fastify.addHook('preValidation', async (request, reply) => {
        ...
        if (request.headers) {
            for (let key in request.headers) {
                ...
                if (key === 'user-info') {
                    try {
                        request.userInfo = JSON.parse(security.base64Decode(request.headers[key]))
                    } catch (e) {
        ...
        // 补充使用模式
        if (request.reqParams.schemaCode && req.reqParams.schemaCode != 'undefined') {
            request.schemaCode = request.reqParams.schemaCode
        } else {
            request.schemaCode = defaultSchema
        }
    })
})
