...
let server = new grpc.Server({
    'grpc.max_send_message_length': process.env.max_message_length ? parseInt(process.env.max_message_length) : 2560 * 1024 * 1024,
    'grpc.max_receive_message_length': process.env.max_message_length ? parseInt(process.env.max_message_length) : 2560 * 1024 * 1024,
})
/**
 * 2. 服务和实现方法添加到gRPC服务器中
 */
let path = doc.baseDir + 'grpc/servers'
...
server.bindAsync(`${process.env.GRPC_ADDRESS || '0.0.0.0'}:${process.env.GRPC_PORT || 50051}`, grpc.ServerCredentials.createInsecure(), (err, port) => {
    if (err != null) {
        doc.logger.error(err)
        process.exit(1)
        return
    }
    doc.logger.info(`服务器启动成功，端口：${port}。`)
