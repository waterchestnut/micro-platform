...
export class ClientDac extends BaseDac {
    ...
    assembleParams(options) {
        ...
        if (tools.isArray(options.clientType)) {
            params.$and.push({clientType: {$in: options.clientType}})
        } else if (options.clientType) {
            params.$and.push({clientType: options.clientType})
        }
        if (tools.isArray(options.endpointType)) {
            params.$and.push({'endpoints.endpointType': {$in: options.endpointType}})
        } else if (options.endpointType) {
            params.$and.push({'endpoints.endpointType': options.endpointType})
        }
